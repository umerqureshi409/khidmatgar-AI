"""
HIFAZAT Agent — Guardian, Error Recovery & Edge Case Handler
Role: Monitor for failures, trigger fallbacks, handle contradictions and missing data.
This agent ONLY activates when something goes wrong or edge cases are detected.
Demonstrates the system's robustness and adaptability.
"""
import datetime
...
def run_hifazat_guard(
    ...
    6. Emergency at midnight → special provider routing
    ...
    if providers is not None and len(providers) == 0:
        ...
        recovery_actions.append({
            ...
            "description": "Searching in wider area (5km → 15km radius)",
            "status": "ATTEMPTED"
        })
        recovery_actions.append({
        ...
        trace = (
            ...
            f"Recovery: suggested wider radius + next-day options + alternative services."
        )
    ...
    if is_service_unknown or needs_clarification:
        ...
        observations.append(f"Service type or location could not be determined.")
        recovery_actions.append({
            "action": "CLARIFICATION_REQUEST",
            "description": "Asking user to specify service type or location",
            "status": "ACTIVE"
        })
        if is_service_unknown:
            message = _clarification_message(lang)
        else:
            message = _location_clarification_message(lang)
    ...
    # --- Scenario 4: Emergency at late hours ---
    current_hour = datetime.datetime.utcnow().hour + 5  # PKT = UTC+5
    if urgency >= 9 and (current_hour >= 23 or current_hour <= 5):
        triggered_by.append("EMERGENCY_AFTER_HOURS")
        observations.append(f"Emergency request at {current_hour}:00 PKT — after-hours scenario")
        recovery_actions.append({
        ...
        trace = (
            f"HIFAZAT activated: Emergency after-hours request at {current_hour}:00 PKT. "
            f"Routing to 24/7 providers only. Emergency surcharge applied."
        )
...
def _timeout_message(lang):
    if lang == "URDU":
        ...
        return "Internet slow hai — main apne database se best providers dkha raha hun."
